...
BEGIN
  ...
  LOOP
    ...
    IF r.nama LIKE '%vocal%' OR r.nama LIKE '%vokal%' THEN skor := skor + lookup_nilai(grade_v) * bobot_v;
...
AS $function$
  ...
  SELECT p.id AS peserta_id,
         ...
         p.nama,
         p.asal,
         COALESCE((SELECT juri_total_sum FROM agg WHERE agg.peserta_id=p.id), 0)::numeric AS total_skor,
