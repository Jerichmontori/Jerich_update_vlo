ALTER TABLE public.kategori
  ADD COLUMN IF NOT EXISTS gunakan_pita boolean NOT NULL DEFAULT true;

DROP FUNCTION IF EXISTS public.get_pita_nilai(text);

CREATE OR REPLACE FUNCTION public.get_pita_nilai(_kategori text)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT jsonb_build_object(
    'gunakan', COALESCE((
      SELECT k.gunakan_pita
      FROM public.kategori k
      WHERE lower(trim(COALESCE(NULLIF(k.kriteria_peserta,''), NULLIF(k.kategori,''), '')))
          = lower(trim(COALESCE(_kategori,'')))
      ORDER BY k.updated_at DESC NULLS LAST LIMIT 1
    ), true),
    'pita', COALESCE(jsonb_agg(to_jsonb(p) ORDER BY p.kategori, p.clear_text, p.urutan), '[]'::jsonb)
  )
...
CREATE OR REPLACE FUNCTION public.admin_set_gunakan_pita(_kategori text, _on boolean)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin') THEN
    RAISE EXCEPTION 'Hanya admin yang dapat mengubah pengaturan pita';
  END IF;
  IF _kategori IS NULL OR trim(_kategori) = '' THEN
    RAISE EXCEPTION 'Kategori wajib diisi';
  END IF;
  UPDATE public.kategori
    SET gunakan_pita = _on, updated_at = now()
    WHERE lower(trim(COALESCE(NULLIF(kriteria_peserta,''), NULLIF(kategori,''), '')))
          = lower(trim(_kategori));
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Kategori tidak ditemukan';
  END IF;
  RETURN jsonb_build_object('ok', true, 'kategori', trim(_kategori), 'gunakan', _on);
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_pita_nilai(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_set_gunakan_pita(text, boolean) TO authenticated;
