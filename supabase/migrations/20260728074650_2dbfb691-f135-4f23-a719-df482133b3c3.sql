...
BEGIN
  ...
  -- Perbedaan bacaan mazmur DINONAKTIFKAN (peserta & mazmur diatur oleh sesi operator)
  ...
  IF perhatian_kid IS NOT NULL THEN
    FOR q_idx IN 1..3 LOOP
      q_idx_ui := CASE q_idx WHEN 1 THEN 1 WHEN 2 THEN 3 WHEN 3 THEN 4 END;
      WITH per_juri AS (
        SELECT juri_id,
               (SELECT jsonb_agg(x ORDER BY ord)
                FROM (
                  SELECT ord, (x)::text AS x
                  FROM jsonb_array_elements(coalesce(detail->'aspek'->q_idx_ui->'ditandai','[]'::jsonb)) WITH ORDINALITY AS t(x, ord)
                ) s) AS marks
  ...
  IF existing IS NOT NULL THEN
    UPDATE public.var_clarification_session
      SET komponen_berbeda = comps, mazmur_id = (SELECT mazmur_id FROM public.penilaian WHERE peserta_id = _peserta LIMIT 1)
      WHERE id = existing;
    RETURN existing;
  END IF;
