...
-- 1. Lookup non-linear untuk grade 1.0 .. 5.0 (9 langkah 0.5)
CREATE OR REPLACE FUNCTION public.lookup_nilai(_grade numeric)
RETURNS numeric
...
DECLARE
  g numeric;
  vals numeric[] := ARRAY[0.050, 0.120, 0.220, 0.360, 0.520, 0.680, 0.810, 0.910, 1.000];
  idx int;
  frac numeric;
BEGIN
  IF _grade IS NULL THEN RETURN 0; END IF;
  g := GREATEST(1, LEAST(5, _grade));
  -- posisi 0..8 di array (0.5 langkah)
  idx := floor((g - 1) / 0.5)::int;
  frac := ((g - 1) / 0.5) - idx;
  IF idx >= 8 THEN RETURN vals[9]; END IF;
  RETURN vals[idx+1] + (vals[idx+2] - vals[idx+1]) * frac;
END;
$$;

-- 2. Hitung nilai akhir per juri per peserta
...
BEGIN
  ...
  -- jitter deterministik ±0.0009 untuk minimalkan tabrakan
...
-- 3. Nilai akhir peserta = rata-rata juri yang sudah submit
...
BEGIN
  ...
  SELECT kategori INTO kat_p FROM public.peserta WHERE id=_peserta;
  SELECT batas_bawah, batas_atas INTO bb, ba FROM public.kategori WHERE kategori=kat_p LIMIT 1;
...
-- 4. Update get_ranking() untuk menambahkan nilai_akhir
DROP FUNCTION IF EXISTS public.get_ranking();
CREATE OR REPLACE FUNCTION public.get_ranking()
RETURNS TABLE(peserta_id uuid, nomor_urut integer, nama text, asal text,
  total_skor numeric, rata_rata numeric, jumlah_juri bigint,
  nilai_akhir numeric, var_status text)
...
GRANT EXECUTE ON FUNCTION public.lookup_nilai(numeric) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.hitung_nilai_juri(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.hitung_nilai_akhir(uuid) TO authenticated, anon;
