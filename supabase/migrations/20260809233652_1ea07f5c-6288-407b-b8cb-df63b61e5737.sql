CREATE OR REPLACE FUNCTION public.set_live_ranking_top10(_on boolean)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
BEGIN
  IF NOT (public.has_role(auth.uid(),'admin') OR public.has_role(auth.uid(),'inspektur')) THEN
    RAISE EXCEPTION 'Tidak diizinkan';
  END IF;
  INSERT INTO public.system_config(key, value) VALUES ('live_ranking_top10', to_jsonb(COALESCE(_on,false)))
  ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = now();
  RETURN jsonb_build_object('ok', true, 'top10', COALESCE(_on,false));
END; $$;

CREATE OR REPLACE FUNCTION public.get_live_ranking_top10()
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  SELECT COALESCE((SELECT (value)::text::boolean FROM public.system_config WHERE key='live_ranking_top10'), false);
$$;

REVOKE ALL ON FUNCTION public.set_live_ranking_top10(boolean) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.get_live_ranking_top10() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.set_live_ranking_top10(boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_live_ranking_top10() TO authenticated;
