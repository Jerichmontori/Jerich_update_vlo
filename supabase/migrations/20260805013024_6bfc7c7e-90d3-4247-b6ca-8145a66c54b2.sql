...
BEGIN
  ...
  -- Aturan khusus: peserta yang pernah masuk VAR / perbaikan (termasuk potensi VAR
  -- yang tidak terpenuhi) memakai acuan nilai berbeda.
  SELECT EXISTS (
  ...
  IF has_var THEN
    ...
    IF clear_text_val IS NOT NULL AND jsonb_typeof(clear_text_val) = 'boolean'
       AND (clear_text_val)::text = 'true' THEN
      -- Clear Text = Ya → acuan nilai tertinggi (batas atas), turun maksimal ke nilai VAR/tidak clear
      out_val := ba - (ba - ns) * pen;
    ELSE
      -- Clear Text = Tidak → mulai dari nilai tengah + hasil catatan juri, dibatasi nilai VAR/tidak clear
      base_val := tg + bonus_ratio * GREATEST(0, ns - tg);
  ...
  IF NOT has_var
     AND clear_text_val IS NOT NULL
