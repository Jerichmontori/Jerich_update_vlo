...
BEGIN
    ...
    per_juri AS (
      SELECT v.juri_id,
             CASE
               WHEN EXISTS (
                 SELECT 1
                 FROM jsonb_array_elements(COALESCE(p.detail->'aspek','[]'::jsonb)) a
                 WHERE jsonb_array_length(COALESCE(a->'ditandai','[]'::jsonb)) > 0
               ) THEN 'false'::jsonb
               ELSE COALESCE(p.detail->'clearText', p.detail->'membacaPerikop', 'null'::jsonb)
             END AS ct
      FROM valid_juri v
  ...
  VALUES (_peserta,
          (SELECT mazmur_id FROM public.penilaian WHERE peserta_id = _peserta LIMIT 1),
          'potensi_var', comps)
...
BEGIN
  ...
  IF aspek IS NOT NULL THEN
    ...
    -- Penandaan ayat kini hanya informasi (tanpa penalti), tetapi memaksa status "tidak clear".
    FOR ay IN SELECT * FROM jsonb_array_elements(COALESCE(aspek->'aspek','[]'::jsonb)) LOOP
